...
int main(int argc, char *argv[]) {
  ...
  for (int l = 0; l < history.size(); ++l) {
    ...
    while (true) {
      ...
      if ((*(tmp.back().begin()) == 0 && *(tmp.back().end() - 1) == 0) || tmp.back().size() == 1)
        break;
    }

    // Calculate values in the begin (avoid vector insert)
    // display_vec(tmp);
    previous = tmp.back().front();

    for (int i = tmp.size() - 2; i >= 0; --i)
      previous = tmp[i].front() - previous;
