...
fn main() {
    ...
    let end_row = grid.iter().max_by_key(|(row, _)| row).unwrap().0 + 2;
    let mut drop = (-1, 500);

    while !sand.contains(&(0, 500)) {
        if drop.0 + 1 == end_row
            || grid.contains(&(drop.0 + 1, drop.1))
            || sand.contains(&(drop.0 + 1, drop.1))
        {
            if (drop.0 + 1 != end_row)
                && !grid.contains(&(drop.0 + 1, drop.1 - 1))
                && !sand.contains(&(drop.0 + 1, drop.1 - 1))
            {
                drop = (drop.0 + 1, drop.1 - 1);
            } else if (drop.0 + 1 != end_row)
                && !grid.contains(&(drop.0 + 1, drop.1 + 1))
                && !sand.contains(&(drop.0 + 1, drop.1 + 1))
            {
                drop = (drop.0 + 1, drop.1 + 1);
...
fn print(grid: &HashSet<(i32, i32)>, sand: &HashSet<(i32, i32)>) {
    ...
    let start_col = sand.iter().min_by_key(|(_, col)| col).unwrap().1;
    let end_col = sand.iter().max_by_key(|(_, col)| col).unwrap().1;
