...
fn main() {
    ...
    while !proceed(&mut orders.clone(), [i, 0, 0, 0]) {
        i += 1;
    }
    println!("Last part: {}", i);
}

fn proceed(orders: &mut [Order], mut registers: [i32; 4]) -> bool {
    let mut output = 1;
    let mut times = 0;
    ...
    while let Some(order) = orders.get(order_index) {
        match *order {
                ...
                if registers[reg] > 0 {
                    if let Some(to_update) = orders.get_mut(order_index + registers[reg] as usize) {
                        *to_update = match to_update {
                            ...
                            Order::Dec(reg) | Order::Toggle(reg) | Order::Out(reg) => {
                                Order::Inc(*reg)
                            }
                        };
                    }
            ...
            Order::Out(reg) => {
                if (output == 1 && registers[reg] == 0) || (output == 0 && registers[reg] == 1) {
                    times += 1;
